...
namespace IMS.Controllers.Implementation
{
    ...
    {       
        public Response CheckForDuplicateCode(Models.Stores.CheckForDuplicateCode.Request.DuplicateCodeCheckInput duplicateCodeCheckInput)
        {
            ...
            Logger.Write(logIdentifier, duplicateCodeCheckInput.ToString(), DateTime.Now.Ticks, LoggerConstants.ITEMS_SERVICE__CHECK_FOR_DUPLICATE_CODE);
            Logger.Write(logIdentifier, classAndMethodName + " START", DateTime.Now.Ticks, LoggerConstants.ITEMS_SERVICE__CHECK_FOR_DUPLICATE_CODE);
            //-Soumee - 29-March-2018 - Adding Loggers
                ...
                using (SqlCommand cmd = new SqlCommand(duplicateCodeCheck, con))
                {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            ...
                            //Soumee - 29-March-2018 - Adding Loggers        
                            Logger.Write(logIdentifier, "Checking for response inside the reader: " + response.IsOk, DateTime.Now.Ticks, LoggerConstants.ITEMS_SERVICE__CHECK_FOR_DUPLICATE_CODE);
                        }
                    }
                }
            }
            ...
            //+Soumee - 29-March-2018 - Adding Loggers
            Logger.Write(logIdentifier, duplicateCodeCheckInput.ToString(), DateTime.Now.Ticks, LoggerConstants.ITEMS_SERVICE__CHECK_FOR_DUPLICATE_CODE);
            Logger.Write(logIdentifier, classAndMethodName + " END", DateTime.Now.Ticks, LoggerConstants.ITEMS_SERVICE__CHECK_FOR_DUPLICATE_CODE);
            //-Soumee - 29-March-2018 - Adding Loggers
